  ...
  return (
    <div style={{ maxWidth: 440, margin: '0 auto', padding: '48px 16px', fontFamily: 'system-ui, sans-serif' }}>
      <div style={{ textAlign: 'center', marginBottom: 40 }}>
        ...
        <p style={{ color: '#666', fontSize: 16, margin: 0 }}>Rodinné hlasování o večeři</p>
      </div>

      {isToday ? (
        <div style={{ background: '#f7f7f7', borderRadius: 16, padding: '20px', marginBottom: 24, textAlign: 'center' }}>
          <div style={{ fontSize: 13, color: '#888', marginBottom: 8 }}>Dnešní hlasování</div>
          <div style={{ display: 'flex', justifyContent: 'center', gap: 8, marginBottom: 12, flexWrap: 'wrap' }}>
            {proposals.map((p: any, i: number) => (
              <span key={i} style={{ background: '#fff', border: '1.5px solid #e5e5e5', borderRadius: 8, padding: '6px 10px', fontSize: 13 }}>
                {p.emoji} {p.name}
              </span>
            ))}
          </div>
          ...
          {todaySession!.winnerName && (
            <div style={{ marginTop: 12, background: '#1a1a1a', color: '#fff', borderRadius: 10, padding: '10px 16px', fontSize: 15, fontWeight: 600 }}>
              🏆 {todaySession!.winnerName}
            </div>
          )}
          <Link href={`/results/${todaySession!.id}`} style={{ display: 'block', marginTop: 12, color: '#666', fontSize: 13, textDecoration: 'none' }}>
            Zobrazit výsledky →
          </Link>
        </div>
      ) : (
        <div style={{ background: '#f7f7f7', borderRadius: 16, padding: '20px', marginBottom: 24, textAlign: 'center', color: '#999' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
        <Link href="/history" style={{ textDecoration: 'none' }}>
          <div style={{ border: '1.5px solid #e5e5e5', borderRadius: 14, padding: '16px 18px', display: 'flex', alignItems: 'center', gap: 12, cursor: 'pointer' }}>
